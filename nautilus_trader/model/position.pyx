...
cdef class Position:
    ...
    cdef double _calculate_pnl(
        ...
        # values.save_value("tick_bid", pytower.tick)
        # values.save_value("id", self.id)
        # values.save_value("quantity", f'{int(quantity)}')   
        # values.save_value("avg_px_open", f'{avg_px_open:.{100}f}')
        # values.save_value("avg_px_close", f'{avg_px_close:.{100}f}')
        # values.save_value("points", f'{points:.{100}f}')
