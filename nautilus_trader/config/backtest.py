...
class BacktestVenueConfig(Partialable):
    ...
    modules: Optional[list[ImportableModuleConfig]] = None  # TODO(cs): Implement
...
class BacktestDataConfig(Partialable):
    ...
    catalog_fs_protocol: Optional[str] = "file"
    ...
    def bar_specification(self) -> BarSpecification:
        return BarSpecification.from_str(self.bar_spec)
    ...
    def get_files(self):
        files = self.catalog().get_files(
            ...
            start=self.start_time_nanos,
            end=self.end_time_nanos,
