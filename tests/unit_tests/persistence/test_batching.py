...
class TestBatchingData:
    ...
    test_parquet_files = [
        os.path.join(PACKAGE_ROOT, "data/quote_tick_eurusd_2019_sim_rust.parquet"),
        os.path.join(PACKAGE_ROOT, "data/quote_tick_usdjpy_2019_sim_rust.parquet"),
        os.path.join(PACKAGE_ROOT, "data/bars_eurusd_2019_sim.parquet"),
    ]
...
class TestBuffer(TestBatchingData):
    def test_buffer_pop_returns_returns_valid_data(self):
        ...
        buffer = Buffer(
            generate_batches(
                files=[self.test_parquet_files[0]], cls=QuoteTick, use_rust=True, n_rows=10
            )
        )
        ...
        batch_gen = generate_batches(
            files=[parquet_data_path], cls=QuoteTick, use_rust=True, n_rows=10
        )
...
class TestGenerateBatches(TestBatchingData):
    ...
    def test_generate_batches_returns_valid_data(self):
        ...
        parquet_data_path = self.test_parquet_files[0]
        batch_gen = generate_batches(
            files=[parquet_data_path], cls=QuoteTick, use_rust=True, n_rows=300
        )
...
class TestIterateBuffersRust(TestBatchingData):
    def test_iterate_single_buffer_returns_valid_timestamps_rust(self):
        ...
        buffers = [
            Buffer(
                generate_batches(
                    [self.test_parquet_files[0]], QuoteTick, n_rows=1000, use_rust=True
                )
            )
        ]

        iter_batches = _iterate_buffers(
            buffers=buffers, target_batch_size_bytes=parse_bytes("10kib"), read_num_rows=1000
        )
    ...
    def test_iterate_multiple_buffers_returns_valid_timestamps_rust(self):
        ...
        buffers = [
            ...
            Buffer(
                generate_batches(
                    [self.test_parquet_files[1]], QuoteTick, n_rows=1000, use_rust=True
                )
            ),
        ]
...
class TestBatchConfigsRust(TestBatchingData):
    ...
    def _load_bars_into_catalog_rust(self):
        ...
        def parser(df, instrument):
            ...
            objs = BarDataWrangler(
                bar_type=BarType.from_str("EUR/USD.SIM-1-HOUR-BID-EXTERNAL"), instrument=instrument
            ).process(df)
            yield from objs
    ...
    def _load_quote_ticks_into_catalog_rust(self):
        ...
        for instrument, parquet_data_path in zip(
            self.test_instruments[:2], self.test_parquet_files[:2]
        ):
    ...
    def test_batch_data_configs_single_config_returns_valid_timestamps_rust(self):
        ...
        iter_batches = batch_configs(
            data_configs=[config], target_batch_size_bytes=parse_bytes("10kib"), read_num_rows=300
        )
    ...
    def test_batch_data_configs_multiple_configs_returns_valid_timestamps_rust(self):
        ...
        base = BacktestDataConfig(
            catalog_path=str(self.catalog.path), data_cls=QuoteTick, use_rust=True
        )
